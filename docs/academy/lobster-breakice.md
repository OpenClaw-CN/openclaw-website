---
title: 龙虾破冰计划 · OpenClaw 入门实战课
description: 5 次直播特训，从零孵化你的专属 AI 私有助理
layout: doc
---

<div class="academy-page academy-page--course">

# 🦞 龙虾破冰计划：OpenClaw 入门实战课

> **5 次直播特训，从零孵化你的专属 AI 私有助理！**
> 在这里，AI 不再是冷冰冰的对话框，而是 7×24 小时待命、懂你心思、替你干活的赛博大龙虾。


## 🎯 这门课适合谁？

不管你是技术大牛还是非科班小白，只要你渴望生产力飞跃，都可以成为优秀的「人类指挥官」：

* 💼 **职场人士**：雇佣一只龙虾助手，打通自动化工作流，让你的工作效率成倍提升，告别无意义的加班。
* 🏠 **全职在家**：只要你还有梦想，一只全能的龙虾足以打破物理空间的限制，助你实现个人价值与搞钱梦想。
* 🤖 **AI 爱好者**：厌倦了只在网页上聊天？带你深层次解剖你的龙虾，掌握底层逻辑，成为硬核养虾高手。
* 💻 **开发者**：也许你觉得跑个开源项目很简单，但如何与自己业务完美契合，这里有你可能真的不知道的工程化坑位。
* ✍️ **自媒体创作者**：让龙虾学习你的爆款网感，自动化搜集素材、处理文案，它是对你最有效的生产力杠杆。


## 🏆 学完你将获得的四大核心战力

摆脱焦虑，让 AI 真正成为你的生产力倍增器：

1. 🦐 **一只拥有灵魂的 AI 私人大龙虾**：7×24 小时待命，永不离线，独属于你的超级大脑。
2. 🔗 **打通国民级社交渠道**：无缝接入飞书、钉钉。无论你在哪里，龙虾都在你的手机里，随时随地听候差遣。
3. 🎭 **绝对忠诚的私人定制**：贴合你的工作人格，服务你的暴躁/温柔性格，它是这个世界上最懂你的数字分身。
4. 💰 **精准匹配的商业场景**：不再为了 AI 而 AI，学会选择最适合你的 Skill（技能）组合，打造能真正为你创造价值的落地产品。


## 🗺️ 5 天硬核实战大纲

**教学模式**：5 次直播 / 10 节大课 / 每节 1 小时。
**服务保障**：导师实时在线连麦答疑，直播均录制高清视频，支持永久回看，专属微信社群长效陪伴。让你的问题不再是问题！

### 🟢 第一天：建虾池（开课与本地部署）
* **第一节**：1.1 安装前准备环境搭建 | 1.2 一键启动你的 OpenClaw
* **第二节**：1.3 运行诊断与排错 | 1.4 日志查看技巧 | 1.5 核心配置修改
* 📚 *课程材料*：《OpenClaw 安装与运维手册》
* 📝 *课后作业*：选择飞书或钉钉作为你的主战场，按照手册完成基础账号配置。

### 🟡 第二天：伸触角（社交通道配置）
* **第一节**：2.1 系统运行深度诊断 | 2.2 飞书/钉钉通道配置实战
* **第二节**：2.3 多场景验证通道交互能力 | 2.4 本地大模型（Olama）怎么配置？
* 📚 *课程材料*：《渠道配置手册》
* 📝 *课后作业*：思考你要让龙虾做什么？你在里面承担什么角色？你们之间的关系是怎样的？

### 🟠 第三天：注灵魂（给助手赋予人格）
* **第一节**：3.1 你是谁？解析 `USER.md` | 3.2 龙虾是谁？解析 `IDENTITY.md` | 3.3 性格捏脸：解析 `SOUL.md`
* **第二节**：3.4 设定工作流：解析 `AGENT.md` | 3.5 安全护栏：解析 `SAFETY.md` | 3.6 往事不随风：如何给龙虾配置长效记忆？
* 📚 *课程材料*：《灵魂配置手册与实操指南》
* 📝 *课后作业*：为你的专属虾注入灵魂、设置安全围栏、并建立第一段有效历史记忆。

### 🔴 第四天：装机甲（龙虾技能 Skills）
* **第一节**：4.1 揭秘 Skills 底层机制 | 4.2 手把手安装你的第一个扩展技能
* **第二节**：4.3 社区最实用的高能技能推荐 | 4.4 最佳的技能组合（Combo）产品方案
* 📚 *课程材料*：《OpenClaw 官方技能组合说明》
* 📝 *课后作业*：根据自己需要的场景，养出一只能真正替你干活的实用龙虾。

### 👑 第五天：登顶之战（结课与斗虾大会）
* **第一节**：**5.1 学员「斗虾大会」！** 参与斗虾的学员在线路演，提交展示你的龙虾私助实战故事与交互截图。
* **第二节**：5.2 结营仪式与颁奖。从斗虾评学员中选 Top 5 最佳龙虾驯兽师，赢取终极大奖！
* 🎁 *结业福利*：**Top 5 优秀学员每人将获得价值 200 元的京东购物卡一张！**


## 👨‍🏫 授课导师：辉哥

> **「不讲虚无缥缈的理论，只教拳拳到肉的落地实操。」**

* 前大厂技术总监
* 资深智能平台研发总监
* 智能体 (Agent) 构建与大模型训练实战专家


## 🚀 立即上车，领养你的龙虾助理！

告别知识焦虑，从环境配置到真正落地，这是你离 AI 最近的一次。

* 📅 **开营时间**：**3 月 28 号 晚上 8:00** 准时发车
* 🦞 **龙虾品种**：学员可自由选择**原版（OpenClaw 官方）**或**中国社区版**
* 💡 **学习保障**：直播回看 + 社群答疑 + 优秀作业奖学金

<div class="academy-course-schedule">

<p class="academy-course-schedule__title">📋 课程时间安排</p>

<figure class="academy-course-schedule__figure">
  <img src="/academy/kechengbiao.png" alt="龙虾破冰计划 · 课程表（开课时间与周期）" loading="lazy" />
  <figcaption class="academy-course-schedule__caption">本次训练营各节直播与周期安排，请以开营群通知为准。</figcaption>
</figure>

</div>

<div class="academy-enroll">

<p class="academy-enroll__lead">微信扫码付费后，添加助理微信进群；请保存好付费凭证截图，便于助理核验。</p>

<div class="academy-enroll__grid">
  <div class="academy-enroll__col">
    <div class="academy-enroll__qr-wrap">
      <img src="/academy/payment-code.jpg" alt="收款码" width="220" height="220" loading="lazy" />
    </div>
    <div class="academy-enroll__meta">
      <div class="academy-enroll__headline academy-enroll__headline--pay" aria-label="早鸟价">
        <span class="academy-enroll__price-num">¥98</span>
        <del class="academy-enroll__price-was">原价 ¥196</del>
      </div>
      <p class="academy-enroll__caption">微信扫码付费。手机端保存图片，微信扫一扫打开即可付款。</p>
    </div>
  </div>
  <div class="academy-enroll__col">
    <div class="academy-enroll__qr-wrap">
      <img src="/academy/assistant-wechat.jpg" alt="助理微信码" width="220" height="220" loading="lazy" />
    </div>
    <div class="academy-enroll__meta">
      <div class="academy-enroll__headline academy-enroll__headline--action">添加微信进群</div>
      <p class="academy-enroll__caption">交流答疑 · 文档资料 · 权益通知</p>
    </div>
  </div>
</div>

<div class="academy-enroll__notice" role="note">
  <span class="academy-enroll__notice-label">重要提示</span>
  <p class="academy-enroll__notice-text">
    扫码付费后，请<strong>添加群助理</strong>，将<strong>付费凭证截图</strong>发给群助理；群助理验证后会<strong>拉你入群</strong>。
  </p>
</div>

</div>

</div>
